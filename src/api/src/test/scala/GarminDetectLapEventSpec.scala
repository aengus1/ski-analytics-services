...
/**
  ...
  *
  */
class GarminDetectLapEventSpec extends FlatSpec with Matchers with BeforeAndAfter {
  var ap: ActivityProcessor = null;
  var lapEvents: Option[List[Event]] = null;

  before {
    val parser: FitParser = new FitParser()
    val stream: InputStream = getClass.getResourceAsStream("/garmin_1.fit")
    parser.parse(stream)
  ...
  "DetectLapEvents for garmin device" should "correctly detect 2 laps in garmin_1.fit" in {

    println("lap events" + lapEvents)
    assert(lapEvents.isDefined && lapEvents.get.size == 2)

  }

  "DetectLapEvents for suunto device" should "correctly determine start time and duration  of lap 1  in garmin_1.fit" in {

    println(lapEvents)
    assert(lapEvents.get(0).ts == "12-05-2017 16:16:39")
...
//    val parser: FitParser = new FitParser()
//    val stream: InputStream = getClass.getResourceAsStream("/garmin_2.fit")
//    parser.parse(stream)
//
//
//    val builder: SafBuilder = new SafBuilder()
